...
def login_user(username, password):
    if username in st.session_state.user_accounts:
        if st.session_state.user_accounts[username]['password'] == password:
...
def show_login_page():
    ...
    <div style="text-align: center; padding: 2rem 0;">
        <h1 style="color: #667eea;">🔔 Appointment Reminders System</h1>
        <p style="color: #666; font-size: 1.2rem;">Please login to continue</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        st.markdown("""
        <div class="login-container">
            <h2 style="text-align: center; margin-bottom: 2rem;">Register New User</h2>
        """, unsafe_allow_html=True)
        ...
        with st.form("register_form"):
            ...
            reg_email = st.text_input("Email", placeholder="Enter email address")
            reg_phone = st.text_input("Phone", placeholder="+91-XXXXXXXXXX")
            reg_role = st.selectbox("Role", ["patient", "staff", "doctor", "admin"])
            reg_department = st.text_input("Department", placeholder="Enter department/specialty")
            
            register_button = st.form_submit_button("Register", use_container_width=True)
            
            if register_button:
                if all([reg_username, reg_password, reg_name, reg_email, reg_phone, reg_department]):
                    if register_new_user(reg_username, reg_password, reg_role, reg_name, reg_email, reg_phone, reg_department):

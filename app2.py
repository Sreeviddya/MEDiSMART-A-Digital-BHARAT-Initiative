...
st.markdown(
    ...
    .stButton[data-testid="stButton-top_guest_user"] > button:hover {
...
def show_navbar():
    ...
    with col2:
        ...
        st.markdown(f'<h3 class="main-tagline">{lang_data["tagline"]}</h3>', unsafe_allow_html=True)
    ...
    with col4:
        if st.button(lang_data["register"], key="top_register"):
            ...
            st.rerun()
    with col5:
        if st.button(lang_data["guest"], key="top_guest_user"):
            st.session_state.current_page = "guest_user"
            st.rerun()
...
def show_home_page():
    ...
    features_data = {
        ...
        "தமிழ்": [
            ...
            ("டிஜிட்டல் சுகாதார பதிவுகள்", "ஆதார் அல்லது பிற அடையாளங்களுடன் நோயாளி வரலாற்றை அணுகுங்கள்."),
...
if st.session_state.current_page == "home":
    ...
    register.show_page()
elif st.session_state.current_page =="guest_user":
    guest.show_page()
    ...
    diabetes.show_page()
elif st.session_state.current_page == "parkinsons_disease":  # FIX APPLIED HERE
